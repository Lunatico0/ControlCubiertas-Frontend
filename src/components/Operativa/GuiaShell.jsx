...
export const H2 = ({ id, children }) => (
  <h2 id={id} className="mt-12 pb-3 text-[26px] font-bold" style={{ fontFamily: "var(--font-display)", color: "var(--tx)", borderBottom: "1px solid var(--bd)", scrollMarginTop: 24 }}>{children}</h2>
)
export const H3 = ({ children }) => <h3 className="mb-2 mt-[26px] text-[17px] font-semibold" style={{ fontFamily: "var(--font-display)", color: "var(--tx)" }}>{children}</h3>
...
const GuiaShell = ({ sidebarTitle, badge, eyebrow, title, intro, toc, backTo = "/", backLabel = "Volver a la app", footer, children }) => {
  ...
  return (
    <div data-app-theme={isDarkMode ? "dark" : "light"} className="flex h-full w-full overflow-hidden text-left" style={{ background: "var(--bg)", color: "var(--tx)", fontFamily: "var(--font-sans)" }}>
      ...
      <aside className="hidden h-full w-[270px] flex-none flex-col overflow-y-auto px-5 py-[26px] md:flex" style={{ background: "var(--sidebar)", borderRight: "1px solid var(--bd-faint)" }}>
        <div className="mb-3 flex flex-col gap-2">
          <BrandLogo height={26} />
          <div className="text-[13px] font-bold" style={{ fontFamily: "var(--font-display)", color: "var(--tx)" }}>{sidebarTitle}</div>
        </div>
        {badge && <div className="mb-[18px] ml-[45px] inline-block self-start rounded-full px-[9px] py-[3px] text-[9.5px]" style={{ fontFamily: "var(--font-mono)", letterSpacing: ".1em", color: "var(--ink-purple)", background: "color-mix(in srgb, var(--ink-purple) 14%, transparent)" }}>{badge}</div>}
      ...
      <main className="min-w-0 flex-1 overflow-y-auto">
        <div className="mx-auto" style={{ maxWidth: 820, padding: "52px 48px 90px" }}>
          <div className="mb-3 text-[11px]" style={{ fontFamily: "var(--font-mono)", letterSpacing: ".14em", color: "var(--ink-lime)" }}>{eyebrow}</div>
          <h1 className="text-[40px] font-bold" style={{ fontFamily: "var(--font-display)", letterSpacing: "-.02em", color: "var(--tx)", lineHeight: 1.05 }}>{title}</h1>
          <p className="mt-4 text-[16px]" style={{ lineHeight: 1.65, color: "var(--tx-3)", maxWidth: 640 }}>{intro}</p>
          {children}
          <div className="mt-14 flex items-center gap-2.5 pt-[22px] text-[12.5px]" style={{ borderTop: "1px solid var(--bd)", color: "var(--tx-7)", fontFamily: "var(--font-mono)" }}>
